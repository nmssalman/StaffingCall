...
<script>
	...
	$(document).ready(function() {
		...
		jQuery('#calendar').fullCalendar({
			...
			editable: false,
			eventLimit: true, // 03:01 pm 04:03 pm
                        //04:03 pm
...
<!--    <label class="css-control css-control-sm css-control-primary css-switch" title="Toggle to switch List view" onclick="changeView();">
        <input type="checkbox" checked="" class="css-control-input"  id="toggleView">
        <span class="css-control-indicator"></span> Calendar View
    </label>-->
...
<!--                                <button type="button" class="btn-block-option" data-dismiss="modal" aria-label="Close">
                    ...
                    <div class="modal-footer">
                        ...
                        &nbsp;
                        <button id="userWorking" style="cursor: pointer;margin-right: 1%;" type="button" class="btn btn-primary">Working
                            <img style="visibility:hidden;" id="workingLoader" width="30" height="10" src="{!! url('/assets/images/loading.gif') !!}" />
                        </button>
                        <button id="userUnavailability" style="cursor: pointer;margin-right: 1%;" type="button" class="btn btn-danger">Unavailable
                            <img style="visibility:hidden;" id="unAvailabilityLoader" width="30" height="10" src="{!! url('/assets/images/loading.gif') !!}" />
                        </button>
                        <button id="userAvailability" style="cursor: pointer;margin-right: 6%;" type="button" class="btn btn-success">Available
  ...
  filter: alpha(opacity=40); /* For IE8 and earlier */
    ...
    <script type="text/javascript">
        ...
        $('#userAvailability').click(function (){
           ...
           $('#userWorking').attr('disabled',true);  
        ...
        $('#userWorking').click(function (){
            
           $('#workingLoader').css('visibility','visible');  
           $('#userAvailability').attr('disabled',true); 
           $('#userUnavailability').attr('disabled',true);
           $('#userWorking').attr('disabled',true);    
           $('#cancelBtn').attr('disable',true);
           ...
           var toDate = availabilityToDate;
            ...
            var requestUrl = '{!! url(Config('constants.urlVar.userCalendarAvailability')) !!}';
            //availabilityStatus:2 means user working somewhere else
             $.ajax({ 
                    url: requestUrl,  
                    type: "POST",
                    data: {_token:CSRF_TOKEN,startDate: fromDate,endDate:toDate,shiftID:selectedShift,availabilityStatus:2}  ,
                    success: function(response){
                        location.href = '{!! url(Config('constants.urlVar.userCalendarView')) !!}';
                    },
                    error:function(e){
